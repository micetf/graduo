 ...
 * @param {string} [props.selectionMode="normal"] - Le mode de sélection ("normal" ou "selection")
 * @returns {JSX.Element} Le composant TimeDisplay
 */
const TimeDisplay = memo(
    ({ values, settings, className = "", selectionMode = "normal" }) => {
                ...
                <div className={`bg-white rounded-lg shadow p-4 ${className}`}>
                    <p className="text-gray-500 text-center">
                        {selectionMode === "selection"
                            ? "Cliquez sur la ligne graduée pour sélectionner des valeurs de temps."
                            : "Passez en mode sélection pour choisir des valeurs à comparer."}
                    </p>
                </div>
            );
        }
